...
`save_booking` is shared by every profile's own `book` tool — see `worker/profiles/hvac.py`.
"""
...
from receptionist.worker.booking.service import CalendarService, NoBooking, SlotUnavailable
from receptionist.worker.lib.datetimes import pretty_day
...
async def save_booking(
    ...
    *,
    ...
    try:
        ...
        call.record.emit("slot_declined", f"{time} on {pretty_day(day)} was not available")
        return await _offer_alternatives(call, day, time)
...
async def _offer_alternatives(call: CallContext, day: str, time: str) -> str:
    """What to say when the caller can't have the time they asked for.

    Booking and rescheduling both land here, so the two can't drift on the wording.
