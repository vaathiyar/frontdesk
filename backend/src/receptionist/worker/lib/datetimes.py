...
The calendar resolves `day` + `time` through here, and so does the suite's in-memory
double, so the two cannot disagree about what "tomorrow at 10" means. The prompt asks the
model for ISO dates, but the natural-language fallbacks stay: a model that slips should
still be understood rather than dropped.
"""
