...
`reschedule` and `cancel` find a booking by. `booking/google.py` stores it as an
extended property and matches it *exactly*. So a caller who
...
def to_e164(raw: str) -> str:
    ...
    **This assumes the digits already carry a country code**, which is what SIP hands over.
    ...
    Anything that isn't a bare digit string passes straight through: a value that already
    carries its `+`, or "" when the caller withheld their number.
    """
    number = raw.strip()
