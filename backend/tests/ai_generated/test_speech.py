...
from receptionist.worker.voice import speech
...
def stt(monkeypatch: pytest.MonkeyPatch) -> type[Recorder]:
    Recorder.last = {}
    monkeypatch.setattr(speech.google, "STT", Recorder)
    return Recorder
...
def tts(monkeypatch: pytest.MonkeyPatch) -> type[Recorder]:
    Recorder.last = {}
    monkeypatch.setattr(speech.google, "TTS", Recorder)
    return Recorder
...
def test_stt_keeps_its_constants(stt: type[Recorder]) -> None:
    speech.build_stt()
    assert stt.last["model"] == speech.STT_MODEL
    assert stt.last["languages"] == speech.STT_LANGUAGES
    assert stt.last["location"] == speech.STT_LOCATION


def test_tts_keeps_its_constants(tts: type[Recorder]) -> None:
    speech.build_tts()
    assert tts.last["voice_name"] == speech.TTS_VOICE
    assert tts.last["model_name"] == speech.TTS_MODEL
