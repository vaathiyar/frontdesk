...
| `…roomConfig.agents[].agentName` | `dispatch-rule-*.json` | Must match `AGENT_NAME` in `agent/worker.py` | Fixed: `receptionist` |
...
|---|---|---|
| `CHAT_MODEL`, `CHAT_EFFORT`, `CHAT_MAX_TOKENS` | `agent/graph.py` | Gemini model, thinking level, output ceiling |
| `MAX_TOOL_ROUNDS`, `RECURSION_LIMIT` | `agent/graph.py` | Cap on tool rounds per turn. Must be passed explicitly — LangGraph's own default is effectively unlimited |
| `STT_MODEL`, `STT_LOCATION`, `STT_LANGUAGES` | `agent/providers.py` | Speech-to-Text model, region, language |
| `TTS_MODEL`, `TTS_VOICE` | `agent/providers.py` | Text-to-Speech model and voice |
| `OPEN_HOUR`, `CLOSE_HOUR`, `SLOT_MINUTES`, `APPOINTMENT_MINUTES` | `services/calendar.py` | Default bookable hours, slot spacing, appointment length |
| `TOKEN_LENGTH` | `links.py` | Signed-link token length — short enough to keep a text to one segment |

Per-business hours are **not** these constants: a profile sets its own `opens`/`closes`
(see `profiles/restaurant.py`, which serves dinner only), and they have to match the hours
